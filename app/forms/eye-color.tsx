...
export default function EyeColorPage() {
	...
	const selectColor = (color: string) => {
		updateFormData("eye_color", color);
		...
		>
			<View style={formStyles.progressContainer}>
				...
				<View style={[formStyles.progressStep, formStyles.progressStepActive]}>
					<Text style={formStyles.progressStepText}>7</Text>
				</View>
				<View style={formStyles.progressStep}>
			...
			>
				<Text style={{ color: formData.eye_color ? "#000" : "#666" }}>
					{formData.eye_color || "Select Eye Color"}
				</Text>
			</TouchableOpacity>
					...
					{eyeColorOptions.map((color) => (
						<TouchableOpacity
							...
							style={[
								...
								formData.eye_color === color && formStyles.selectedOption,
							]}
						...
						>
							<Text
								style={[
									...
									formData.eye_color === color && formStyles.selectedOptionText,
								]}
							>
								{color}
							</Text>
			...
			<TouchableOpacity
				style={[
					...
					!formData.eye_color.trim() && { backgroundColor: "#ccc" },
				]}
				onPress={handleNext}
				disabled={!formData.eye_color.trim()}
			>
